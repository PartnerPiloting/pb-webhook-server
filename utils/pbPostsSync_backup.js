// utils/pbPostsSync.js - MULTI-TENA    const normalizedFormula = `LOWER(SUBSTITUTE(SUBSTITUTE(SUBSTITUTE({${AIRTABLE_LINKEDIN_URL_FIELD}}, "https://", ""), "http://", ""), "www.", ""))`;
    const formula = `OR(${normalizedFormula} = "${normUrl}", ${normalizedFormula} = "${normUrl}/")`;
    console.log(`[getAirtableRecord] Formula: ${formula}`); SUPPORT: Updated to use client-specific Airtable bases
