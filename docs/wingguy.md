...
**▶▶ 2026-07-18 (later) — THE DRAFTING CHAT'S *REVISED* HANDOVER RE-VERIFIED; MOSTLY ALREADY LANDED, 3 NEW ITEMS FOLDED. Do not re-process it.**
- The claude.ai drafting chat handed over a fuller, revised version of its 17-Jul brief (it now includes the false-positive warnings + the method note, but is UNAWARE of the 17-Jul evening fixes). Re-verified against source: **everything actionable in it already shipped 17 Jul** (evening block below) - transcript alerting (sweep is LIVE + scheduled on prod: `FATHOM_POLL_ENABLED=true`, 5-min interval, checked via Render API 2026-07-18), tiered conflict-check, ledger-unit=KEY, history layer display, provisioning ("unknown" = stale; live since 14 Jul, Portal Token is the mint). Its remaining asks were already the STILL-OPEN list (set-piece ledger, price variables, stage-call-numbers, onboarding-proposition awaiting commit).
- **Corrections to the brief (so its wording doesn't re-seed errors):** (a) propose's neighbour retrieval is now THREE-tier (same cell / same type elsewhere / same key anywhere), not just same-type-any-context; (b) no $250 price exists anywhere in the repo (canonical pricing $150/$200/$300) - though rule BODIES live in the store, so a price inside a live rule can't be repo-checked; (c) "dedupe keys on URL" described the pre-fix bug, not a design choice - the unit is the KEY, decided + tested.
- **Genuinely new, folded into STILL OPEN below:** the calendar-vs-recording reconciliation gap; Subbu Iyer's likely-spurious `signup_link` ledger row; the floor-vs-default convention on the layer-precedence item.
- **Durable operational note:** two sessions committed to the store the same day (`day-aware-signoff` foundation v1 landed 07:35 UTC 17 Jul from a concurrent chat) and `expected_version` held - the door's optimistic lock works in anger; never commit without it. Store state at 17-Jul close: `no-repeat-set-pieces` (foundation v1) · `introduction-emails-html` (client v2, added the base shell it never specified: Arial/14px/1.4/#333333, no max-width) · `email-html-format` (client v3 - supremacy clause SCOPED to follow-up/post-call, the self-scoping precedent the layer item cites) · `onboarding-proposition` proposed, NOT committed.
...
- **✅ FIXED + SHIPPED (be992e6b, 553db87e):**
  - **Asset detector URL-twin bug** (live: drafting `cost_benefit_page` also burned `signup_link`'s gate). The literal-URL scan ran over the **resolved** body, so a token's own expansion matched any key sharing that URL. Now scans the ORIGINAL body + boundary-matches URLs (a prefix URL can't claim a longer one - a second bug the brief missed). **The ledger's unit is the KEY.** 6 new tests.
  - **Propose-time conflict check, three bugs:** (a) the "exclude this chain" filter matched rule_key+campaign but NOT layer → a cross-layer same-key twin, *the exact collision worth catching*, was hidden as if it were self; (b) foundation proposals read only the foundation layer (blind to the client rules they render beside) AND lost the caller's tenant (validateRuleInput correctly blanks it) → added `readerTenantId` for read scope; (c) neighbours were same-context+type only → **now tiered: same cell, same type elsewhere, same key anywhere** (loudest - both copies render, nothing shadows). 3 new tests.
  - **Rule history display** - rows carry layer/tenant, the renderer dropped them, so template+client rows read as double-writes.
  - **Transcripts (the highest-priority find):** `fathomRecordingIngested` answered *"has any row been filed?"* = **ATTEMPTED, not SUCCEEDED** → an empty-bodied shell **sealed itself in permanently** (webhook + poller skipped it forever) = how Kate Phillips (1 Jul) was lost past retention. Now requires a non-empty body → the poller retries. **And the root cause is gone: ingest now files NOTHING when there's no transcript** (single AND split paths) — Fathom's transcript often isn't ready when `meeting_content_ready` fires, and filing the header anyway is what made a missing capture look like coverage. Filing nothing = nothing masquerades as a meeting + the poller retries until it lands + it ages out of the list window by itself. (This was also *required*: with dedup meaning SUCCEEDED, filing an empty shell would have re-ingested every 15 min = a new empty row per pass.) Plus: zero-line ingest logged at info as `(0 lines, 1 leads)` → gone; poll failures discarded their reason (only `failed=1` at info, no id, no why - plausibly where Julian's 17 Jul restart vanished) → now logged per-recording ("no transcript yet" counts as a skip, not a failure); **coverage sweep on the poller's existing 15-min heartbeat** (no new cron) + `lastCoverage` on poll status + `scripts/transcript-coverage-check.js` (read-only, on demand); `recall_latest_transcript` now serves the newest meeting that HAS a body and refuses to pass off a hollow header ("missing, not empty").
  - **★ THE SWEEP'S FIRST PROD RUN FOUND 21, NOT 2** (the brief only saw the two it stumbled on). 7 are `fathom-api` rows WITH recording ids (Kate 10216, Phil Purcell 10218, Maria Burwood 10211, April 10210, Lianne Grove 9142, JB 8932, Celeste 7807) — **10211+10210 share `fathom_rec=159279721`, i.e. a back-to-back split where BOTH children came out empty**. The rest are old Recall-era/no-recording rows and calendar noise ("Lunch", "Busy in Sydney", test bots). **NOT actioned — needs Guy:** (a) are the 7 still inside Fathom's retention (re-ingestable), or lost? (b) the ~14 junk rows want a prune (deleting real data = explicit go). Re-run: `node scripts/transcript-coverage-check.js [graceMins]` as a Render job.
- **❌ FALSE POSITIVE - do NOT "fix" it:** the door-log "double-writes" are **not duplicates** - the import fans one seed rule across template AND client layers, so two legitimate history rows land a second apart; `email-html-format` really was committed twice (template 10 Jul, client 13 Jul) and both v2s exist. The audit trail was never wrong; the **window** onto it was (fixed above). **Adding a UNIQUE constraint - the brief's suggestion - would have destroyed legitimate audit rows.**
- **❌ STALE:** its §2.8 "provisioning may not exist / blocks Julian" - Julian has been provisioned and live since 14 Jul (Portal Token IS the mint). That chat was working from old context. Correct in noting **the URL is the credential** (no second factor).
- **★ STILL OPEN (need Guy, not just code):**
  - **LAYER PRECEDENCE (design call, ~1h once decided).** Verified: there is **none** - runtime is a bare concatenation (foundation first, then client, alphabetical), and the renderer **strips rule_key/layer/version**, so the model can't tell a foundation rule from a client one and **a rule cannot reliably claim precedence even in prose** (`no-repeat-set-pieces` currently tries). Same-key cross-layer duplicates ALREADY exist in prod (see `docs/wingguy-rule-retire-spec.md`) and **`seedClientFromTemplate` can manufacture them for every new tenant** (its skip-set checks client rows only - matters for Julian). **Guy's call:** foundation-wins for guarantees but client-wins for voice is the likely shape - it's a three-drawer question, not a code question. **Convention candidate (drafting chat, folded 2026-07-18) - adopt this rather than a blanket order:** every foundation rule declares itself a FLOOR (client can't override - `no-repeat-set-pieces` already says so) or a DEFAULT (client voice wins - `day-aware-signoff` already yields: "their sign-off phrasing rules still apply"); a conflict resolves by a rule SCOPING ITSELF (the `email-html-format` v3 move: it limited its own supremacy clause to follow-up/post-call rather than claiming everything), never by a global order that would let platform voice guidance stamp on every tenant's actual voice. Caveat: prose declarations only bind reliably once the renderer stops stripping layer/key - the two fixes travel together.
  - **SET-PIECE LEDGER (L).** The near-miss that started it: a follow-up to Subbu Iyer reproduced the manifesto near-verbatim 10 days after it went to him. Assets are gated by URL/key; **rule-driven PROSE (manifesto, canonical framings, origin story, stock closers) is ungated.** `{{setpiece:manifesto}}` keys are the right shape - it gives prose an identity so `wingguy_create_draft` can refuse a repeat exactly as it does an asset. Not yet built; `no-repeat-set-pieces` (foundation v1) is guidance in the meantime.
  - **Stage rules keyed off call numbers that don't hold** (Guy: "I'll often go further on call one"). `no-repeat-set-pieces` moves the trigger to the written record, so it can't cause duplicate prose - but call1/call2/call3 rules still READ as a fixed sequence. **Not urgent; do NOT bulk-rewrite.**
  - **Price variables** (`first_month_price`, `monthly_price`) - `onboarding-proposition` avoids hard-coding but no variables exist to use (S).
  - **`onboarding-proposition`** was PROPOSED, not committed (client, post-call/stage-logic, expected_version=0) - awaiting Guy's yes.
  - **No-op commit guard:** the door has none, so re-committing an identical body silently churns a version (the dedupe lives only in the import script). Latent, small.
  - **Calendar-vs-recording reconciliation (the sweep's blind side; folded 2026-07-18).** The coverage sweep only flags rows that EXIST with empty bodies - and since the 17-Jul fix, ingest files NOTHING when there's no transcript, so a wholly-missed capture leaves NO row and nothing fires. That's exactly Julian's 17-Jul loss (call dropped; only a 3-min aborted recording `164649498` exists in Fathom; the restart was never recorded - nothing for the poller to retry). Closing it = a daily reconcile of calendar events vs captured recordings; the calendar read seam (`listEventsForCoach`, all providers) now exists to build on. Not built.
  - ~~Subbu Iyer's `signup_link` ledger row~~ **✅ RESOLVED 2026-07-18 (Guy's go, deleted via Render job).** The pre-fix URL-twin bug had logged `signup_link` alongside `cost_benefit_page` on his 17-Jul draft (same draft_id `r-7785980271750862638`, same-millisecond burst - confirmed the artifact before deleting). Row id=20 deleted by id; his 3 legitimate rows (consulting_mrr_article, cost_benefit_page, network_grew_network_deck) untouched. A future "here's where to join" email to him now passes the gate normally. **If any OTHER lead was drafted `cost_benefit_page` (or another URL-twin) before the 17-Jul fix, they'd carry the same artifact - none known, but that's the check if a false gate-refusal ever surfaces.**

**▶▶ 2026-07-17 (later still) — ONBOARDING CHECKLIST + MEETING-LINK POLICY + THE LIVE-CALL TOOL.** `docs/wingguy-onboarding-checklist.md` = the end-to-end new-client runbook, restructured as Guy's **live-call script** (read-aloud overview paragraph per step + per-step expansions with "Say to the client" verbatim lines / "You do" / "The client does" / checks / traps). **`wingguy_onboarding_guide` MCP tool shipped** (wingguyGetStartedMcp.js): in any claude.ai chat Guy says "give me the onboarding overview" / "expand step 3" and it serves the CURRENT doc straight from the deployed repo (no stale copies — the doc stays the single source of truth); **owner-gated** (a client calling it gets redirected to wingguy_get_started); ⚠ Guy must RECONNECT his connector to see it. Also: runbook (record → connector → rules → calendar incl. the Zoho account trap + Read IDs=all → meeting link → email → Fathom → close-out). **Meeting-link policy DECIDED: one reusable personal link on every invite (PMI / reusable Meet/Teams link), waiting-room on — per-meeting link generation deliberately NOT built** (Nylas conferencing autocreate = the known small build if a client ever insists; Zoom-per-meeting = a whole OAuth integration, only if a paying client demands it). Julian's `Calendar Read IDs=all` set (verified live — his wrong-account grant still shows 1 calendar/1 event, as expected until he re-auths); "more good news" multi-calendar email drafted for Guy to send.
...
- **★ THE MODEL:** read MANY / write ONE, driven by two new roster fields (Master Clients, `scripts/add-multi-calendar-fields.js`, **idempotent, not yet run — run it as a Render job or locally with a key**):
  - **`Calendar Read IDs`** — blank = the provider default calendar (EXACTLY the old behaviour, so every existing tenant is untouched) | `all` = every calendar in the connected account (Nylas skips read-only subscribed feeds — FYI noise; list one explicitly to include it; google can't enumerate — service accounts only see explicit shares, so list emails) | comma-separated provider-native ids.
  - **`Calendar Write ID`** — blank = provider default (old behaviour) | one id: bookings + holds land THERE (fed to the existing `calendarUid` (zoho) / `nylasCalendarId` (nylas) seams, which existed but nothing ever populated).
- **What changed where:** `calendarProvider.js` — `parseReadIds` + multi-calendar loops in all three read branches (events tagged `calendarId`, deduped on summary|start|end across calendars), new **`listCalendars(coach)`** export, Zoho `"No events found."` sentinel dropped EXPLICITLY (was surviving by luck); `getBatchAvailability` (google) takes an id ARRAY — freebusy natively merges multiple calendars in ONE call, events/meetingCount stay primary-calendar-only (daily load = client calls, not gym sessions); `wingguyCalendar.js` — roster fields fetched (fields[] selection REMOVED so a deploy never 422s on a not-yet-created field), `googleReadSet`, clash check reads the extra calendars too; `clientService.js` maps both fields (write id feeds both provider names).
- **★ ALL-DAY EVENTS (the folded-in Zoho gap, now ALL providers):** mappers keep them with `allDay: true` pinned to COACH-tz midnights (google date-only / nylas `when.date`+`datespan` / zoho bare `yyyyMMdd`) — but `getMeetingsInWindow` DROPS them by default, so availability, clash checks, holds and the Fathom splitter never see them (an all-day "Leave" can't blanket-block a day — the exact regression the doc feared). ONLY `listEventsForCoach` opts in (`includeAllDay`), and the MCP formatter renders `ALL-DAY` / `ALL-DAY (16 Jul → 20 Jul)` instead of a nonsense "12:00 am–12:00 am". `buildDaysFromBusy` also guards directly.
- **⚠ VERIFY-LIVE (both flagged in code):** Nylas `datespan` end_date and Zoho bare-date end are ASSUMED exclusive (Google semantics) — check against a real multi-day all-day event when one first shows up in a listing.
- **Tests:** `tests/wingguy-multi-calendar.test.js` (27 green: field grammar, dedup, tz-midnight spans, all three all-day mappers, sentinel, blanket-block guard); all existing wingguy suites re-run green.
- **★ VERIFIED ON PROD (2026-07-17, read-only Render jobs; commits 2cee949e + cd5e3416 live; roster fields CREATED via the field script):**
  - **Guy-Wilson (google, fields blank):** availability pipeline unchanged (21 days, plausible counts); listing now shows his REAL all-day event — `ALL-DAY (16 Jul → 19 Jul)  Ashtoms House Sitting` rendered correctly by the MCP formatter, appearing on EVERY day it covers, exclusive-end midnight correctly pulled back a day — while Sat 18 still shows 16 free slots (all-day did NOT blanket-block).
  - **Julian-Davis (zoho, via the wrong-account grant — used as the live Zoho test bed before he re-auths):** `listCalendars` works against real Zoho (one calendar "me" [DEFAULT] + uid), availability + listing flow through the zoho path (his single test event shows), reconfirming the wrong-account diagnosis in passing.
  - Check scripts: `scripts/wingguy-multi-calendar-check.js <clientId>` (fields → listCalendars → availability summary → listing) and `scripts/wingguy-list-calendars.js <clientId>` = the setup step that fills the fields.
- **NEXT:** (1) when Julian re-auths on `xapi.com.au`: diagnostic first (unchanged advice), then set his `Calendar Read IDs=all` + nominate his write calendar — the multi-calendar ghost the 2026-07-17 close warned about is now a FIELD EDIT, not a build; (2) same offer to Guy if he wants his personal calendar read (list emails explicitly — google can't `all`); (3) the two VERIFY-LIVE datespan assumptions when a real multi-day all-day event first lists on nylas/zoho.

**▶▶ 2026-07-17 SESSION CLOSE — JULIAN IS LIVE ON CHAT + RULES; HIS CALENDAR IS CONNECTED TO THE WRONG ZOHO ACCOUNT. START HERE.**

- **★ WHERE JULIAN ACTUALLY IS:** connector **connected + working**, rules **set up**. Calendar **connected but to the WRONG Zoho account** (see below). Email (Nylas) **not connected**. Fathom **not connected**. `Meeting Link` on his record still **BLANK** → his invites would go out with no join link (Guy to fill).
- **★ THE WRONG-ACCOUNT DIAGNOSIS (the session's main find — don't re-derive it).** He reported "Wingguy sees my calendar but says no events". It was **not a bug**: he authorised his personal **`me@juliandavis.com`** Zoho, not his working **`julian@xapi.com.au`** account. Zoho's consent screen binds to *whoever is logged in at the time* — that's the trap, and it will catch every future client. **Proof:** across **30 days back → 14 forward** his connected account returned exactly ONE event (a test event he made minutes earlier); the whole prior month came back `{"message":"No events found."}`. Meanwhile Guy's own calendar shows their real 17-Jul meeting inviting `julian@xapi.com.au` — an event that **does not exist** in the connected account. **Fix:** he logs out of Zoho (or uses a private window), re-clicks `/auth/zoho/start?clientId=Julian-Davis&token=<his Portal Token>`, and confirms the consent screen says `julian@xapi.com.au`. Re-running is safe/idempotent (`prompt=consent` forces a fresh refresh token and overwrites the record). An email draft for this was written for Guy to send.
- **⚠ NEXT-GHOST WARNING — check this the moment he reconnects.** `getZohoCalendarUid` uses `coach.calendarUid` if set, **else the `isdefault` calendar, else `cals[0]`** — and **nothing ever populates `calendarUid`** (`getCoachCalendarInfo` doesn't read such a field). So if his real `xapi.com.au` account has SEVERAL calendars and his meetings aren't in the default one, we'd silently read+write the wrong one and reproduce the **exact same "no events" symptom from a totally different cause**. `scripts/wingguy-zoho-diagnose.js` lists every calendar + its `isdefault`, so ONE run after he reconnects rules this out. Do that before believing it's fixed.
- **★ TESTING CAPABILITY (built this session — use it, don't rebuild it).** Any Wingguy tool can be run **as any tenant** against prod, from here, via a Render one-off job (see memory `reference_render_jobs_exec`). Two read-only scripts, both taking a clientId: `scripts/wingguy-list-events-check.js <clientId>` (exercises the real tool path incl. formatting) and `scripts/wingguy-zoho-diagnose.js <clientId> [daysForward] [daysBack]` (record state → token refresh + granted scopes → every calendar → raw events per ≤30-day chunk → which events the mapper keeps/drops + the first dropped event's full raw shape). **This beats a connector for testing**: it runs the LATEST deployed code, so the MCP tool-list cache never confuses results. Also `scripts/wingguy-julian-preflight.js`. **Reads are free; writes hit a real client's real data — get an explicit go.**
- **★ DECIDED (not built): MULTI-CALENDAR = READ-MANY / WRITE-ONE.** Today it's **one calendar for BOTH read and write** — `getViaZoho`/`createViaZoho`/`deleteViaZoho` all call the same `getZohoCalendarUid` → his Zoho default. (Nylas is the same shape: `nylasCalendarId || 'primary'`, also never populated per-client.) The right model is **read every calendar** (this is what makes the no-double-book guarantee TRUE — miss his personal calendar and Wingguy books a lead over his dentist) and **write to ONE nominated calendar** (so "where did that meeting go?" has a boring answer). **Effort:** pinning the write target is nearly free — the `calendarUid` seam already exists at `calendarProvider.js:290`, it just needs a field + one line in `getCoachCalendarInfo`; multi-read is a contained change to `getViaZoho` (it already fetches the calendar list). **Guy's call: WAIT** until we see what his real account looks like — one diagnostic run says whether this is real or hypothetical.
- **★ ZOHO API FACT (durable):** an empty window returns **`events: [{"message": "No events found."}]`** — a sentinel object inside the array, **not** an empty array. Harmless today *by luck*: it has no `dateandtime`, so `mapZohoEvent` returns null and it drops. Worth handling explicitly so it can never be miscounted as a real event.
- **★ ALSO SHIPPED THIS SESSION:** (a) **extension gate moved from env var → the `Wingguy Enabled` (Yes/No) record field** — flip a client on/off in Airtable, no redeploy; closed-by-default; owner always enabled in code; `WINGGUY_ENABLED_CLIENTS` **deleted from prod**. (b) **Claude 529/overloaded handling** — `maxRetries` 2→4 on the shared Anthropic client (SDK auto-retries 429/5xx/529 with backoff), and the `/chat` route now returns a calm "briefly busy, try again" instead of a raw `529 {…}` payload; the extension shows friendly sentences as-is and cleans up any raw error. **⚠ The extension half needs an extension RELOAD to take effect.**
- **⚠ RECONNECT REQUIRED (both Guy and Julian):** `wingguy_list_events` is new; MCP tool lists cache at connection init, so neither will see it until they disconnect/reconnect the Wingguy connector and start a fresh chat. **A plausible second cause of Julian's "no events"** is that his Claude had no list tool and fell back to `wingguy_check_availability` (free slots → "nothing on").
- **★ NEXT:** (1) Julian re-connects Zoho on the RIGHT account → **run the diagnostic** (rules out the multi-calendar ghost + confirms his real events). (2) Both reconnect the connector. (3) Guy adds Julian's Meeting Link. (4) Then: email (Nylas/IMAP) → Fathom. (5) Optional-when-real: multi-calendar read-many/write-one; the Zoho all-day gap (below).

**▶▶ 2026-07-17 — `wingguy_list_events` SHIPPED ("what's on my calendar?" inside Wingguy).**
- **Why:** Wingguy could offer/verify/book times but had NO way to answer "what's on today?" — so that question silently fell through to a client's **own Google Calendar connector**. Works for Guy; **impossible for Julian** (Zoho has no claude.ai connector — the whole reason the adapter exists). This closes the last "everything through Wingguy" gap for calendar reads.
- **What:** `listEventsForCoach(clientId, {range|date|endDate})` in `services/wingguyCalendar.js` reads through the SAME provider seam booking uses (google | nylas | zoho) and **owns the date/tz math** (`range` = today | tomorrow | this_week | next_week, resolved in the coach's own timezone; or explicit `date`/`end_date`). `wingguy_list_events` in `wingguyBookingMcp.js` is a thin formatter (groups into the coach's local days) — read-only, auto-registers on both transports. Commits 63c6d312 → 07a861cf.
- **★ VERIFIED LIVE against Guy's REAL Google calendar** (`scripts/wingguy-list-events-check.js <clientId>`, via a Render job). The live check **caught two real display bugs**, both fixed + re-verified: (a) a multi-day event (16→20 Jul house-sitting) was grouped under its START date, so "today" printed a day OUTSIDE the window — now we iterate the days asked for and list events **overlapping** each; (b) time-only display collapsed it to a nonsense "10:00 am–10:00 am" — now spanning events render `ALL-DAY/MULTI-DAY (runs 16 Jul 10:00 am → 20 Jul 10:00 am)`, while an event ending at exact midnight (his daily "Dinner", 5:30pm–12:00am) counts as same-day, not multi-day.
- **⚠ NOT tested on Zoho yet** — Julian hasn't connected his calendar, and the throwaway Zoho account is gone. Same seam, so it *should* work; re-run the check script with `Julian-Davis` the moment he connects.
- **⚠ KNOWN GAP — Zoho all-day events won't list.** `mapZohoEvent` returns `null` for date-only (all-day) events — `zohoToISO` only parses `YYYYMMDDTHHMMSS(Z|±ZZZZ)`, not a bare `YYYYMMDD`. Correct for clash-detection (an all-day "Leave" marker isn't a 30-min clash) but wrong for *listing*. **Deliberately NOT fixed yet:** the fix must add an `allDay` flag AND filter it out of the availability path (`buildDaysFromBusy`/`countRealMeetings`/`clashesForWindow`) or an all-day event would blanket-block a whole day's slots — a regression in the guarantee-heavy booking path, unverifiable until a real Zoho calendar exists to test against. Do it WITH Julian's live Zoho, verifying both listing AND unchanged availability.
- **NEXT:** Guy + Julian must **reconnect the connector** to see the new tool (MCP tool lists cache at connection init). Once Guy's happy, he can drop his separate Google Calendar connector → the ONE-CONNECTOR design.

**▶▶ 2026-07-16 (later) — HISTORY-DEPENDENT RULES NOW ENFORCEABLE: asset ledger + usage gate + narrow inbound check.**
- **The gap:** the asset-usage-gates rules say "check email history before sending anything twice" — but Wingguy was WRITE-only for email, and the only mailbox read path was the Gmail connector on Guy's personal OAuth. For every other tenant those rules were inert. Fix chosen: Wingguy records what IT sent (no mailbox read at all) + ONE narrow, typed inbound question — deliberately NOT generic mailbox search (`search_query_native` rejected: Google/Microsoft restrict which params combine with it, IMAP has no equivalent — it does not abstract across providers).
- **Asset ledger (`wingguy_asset_ledger` in the rules store):** one row per (lead × asset) at DRAFT time — tenant_id, lead_email, asset_key, draft_id, thread_id, subject, sent_at. `wingguy_create_draft` now: resolves `{{asset:key}}` tokens in html_body to the library's stored URL (unknown key = refused, library listed), recognises LITERAL library URLs pasted in, **REFUSES a draft repeating an asset to the same lead** ("the asset-usage gate") unless `resend_ok: true`, and logs ledger rows on success (best-effort — a logging failure never kills a created draft). Store functions: `recordAssetSends` / `getLeadAssetHistory` / `getAssetSendSummary`.
- **New tools (both transports, auto-registered from TOOL_DEFS):** **`wingguy_lead_history`** (lead_email → every asset already drafted to them, dates + subjects — the check behind the gates; ledger starts 2026-07-16, earlier sends absent) · **`wingguy_lead_replied_since`** (lead_email + since_iso → YES with last inbound message/messageId, or NO; Nylas Messages list filtered by `from` + `received_after` on the coach's OWN grant — Gmail/Outlook/IMAP alike).
- **Zoho/IMAP flags (vs the OAuth connect flow):** (a) IMAP grants are minted via app-specific password + Nylas custom/IMAP auth, not hosted OAuth; (b) IMAP messages are served from Nylas' **~90-day rolling cache** — `wingguy_lead_replied_since` auto-retries with `query_imap=true` (live IMAP query) when the window is older than 90 days and the cache came back empty; OAuth grants never need it.
- **Tests:** `tests/wingguy-asset-ledger.test.js` (pure detectAssets + fake-pool ledger, all green). **Verify on prod:** `scripts/wingguy-asset-ledger-test.js` (one-off Render job) — throwaway asset → draft-with-token → gate-refusal → resend_ok override → history → replied_since; deletes its own ledger rows, retires the test asset, leaves 2 drafts to bin.
- **PERSON-SCOPED READ added same session (Guy's ask: "read me their reply / show my emails with X"):** **`wingguy_lead_correspondence`** (both directions with ONE person via Nylas `any_email` — date/subject/snippet/direction/messageId, newest first, optional since_iso with the same >90-day `query_imap` retry) + **`wingguy_read_message`** (one full body by message id, HTML rendered to readable text via `htmlToText`, capped ~6k chars). `mailProvider` gained `getMessage` + `anyEmail`/`fromEmail`/`to` on findMessages. **The line held:** person-scoped typed filters only — whole-mailbox KEYWORD search stays OUT (provider semantics diverge; blast radius). Tests `tests/wingguy-mail-read.test.js`; prod proof `scripts/wingguy-mail-read-test.js` (read-only Render job).
- **⚠ Connector cache:** reconnect the Wingguy connector in claude.ai to see the 4 new tools + the `resend_ok` param.

**▶▶ 2026-07-16 — DRAFTS CAN NOW THREAD: reply_to_message_id + wingguy_find_message.**
- **The gap (surfaced replying to Jason Hartley in claude.ai):** `wingguy_create_draft` had no threading param, so every draft was a standalone message; a conversational reply forced a fallback to the Gmail connector — which drags the google.com/url link-wrapping back in. A threaded reply WITH links had no clean path at all.
- **The fix (services/mailProvider.js + services/wingguyMailMcp.js):** optional `reply_to_message_id` on `wingguy_create_draft`, passed through to Nylas' native `reply_to_message_id` (Nylas sets In-Reply-To/References and files the draft on the thread). Plus a new read-only **`wingguy_find_message`** tool (search by from/subject/thread_id via the coach's own Nylas grant) that supplies the message id — so the reply flow works on ANY provider (Gmail/Outlook/IMAP — matters for Julian's Zoho-over-IMAP mailbox) and never depends on the Gmail connector's ids. Both tools auto-register on both transports from TOOL_DEFS.
- **Verify on prod:** `scripts/wingguy-thread-draft-test.js` (one-off Render job) — finds a real message, drafts a linked reply onto it, reads it back, checks draft `thread_id` == original's. Leaves the draft for Guy to bin.
- **⚠ Connector cache:** claude.ai caches a connector's tool list at connection init — Guy must RECONNECT the Wingguy connector to see the new param + tool.

**▶▶ 2026-07-14 (later) — EXTENSION GATE MOVED FROM ENV VAR TO A RECORD FIELD.**
- The Chrome-extension allow-list is no longer the `WINGGUY_ENABLED_CLIENTS` env var — it's a per-client **"Wingguy Enabled" (Yes/No)** field on the Master Clients roster (`tblDGthsiHuLIAsGP` in `appJ9XAZeJeK5x55r`), read into `req.client.wingguyEnabled`. `requireOwner` in `routes/wingguyRoutes.js` now passes a client on `cid === OWNER || wingguyEnabled`; **closed by default; the OWNER is always enabled in code** (a field edit can't lock Guy out). So switching a client's extension on/off is now an **Airtable edit, no redeploy**. Field added + Julian stamped Yes via `scripts/add-wingguy-enabled-field.js --commit` (idempotent; it also migrated the old env list); `services/clientService.js` reads it beside `managedClaudeKey`; the env var was **deleted from prod**. Verified live: Julian `wingguyEnabled=true`, Guy still enabled. NB it's a ROSTER field so Master-Clients-only (no Client Template copy — the template is a leads base). The **connector** gate is unchanged (Active client + matching Portal Token; `WINGGUY_CONNECTOR_MULTITENANT=1` global). Historical mentions of `WINGGUY_ENABLED_CLIENTS` below are SUPERSEDED by this.
...
- **★ ZOHO CALENDAR ADAPTER: BUILT + PROVEN AGAINST A REAL ZOHO ACCOUNT.** Full build shipped this session (see `docs/zoho-calendar-adapter-plan.md` for the checklist, all items done):
  ...
  - OAuth connect flow `routes/zohoAuthRoutes.js` (`/auth/zoho/start` + `/callback`, Portal-Token-authorized, HMAC-signed state). Env: `ZOHO_CLIENT_ID`/`ZOHO_CLIENT_SECRET` + `ZOHO_REDIRECT_URI` + `ZOHO_ACCOUNTS_BASE=https://accounts.zoho.com.au` (AU app, registered in Tania's throwaway Zoho — one app serves ALL zoho tenants; swap the two values later if moving to a business account, names stay).
  - **PROVEN LIVE on a real AU Zoho account** (throwaway, now deleted): connect flow → read availability (events mapped, AU timezones correct) → booking via the adapter created a real event AND emailed the external guest. One live fix: Zoho rejects an empty `attendees` array, so guest-less events (HOLDs) omit the field (Guy doesn't use HOLDs — skipped further).
- **★ JULIAN FLIPPED ON (prod, today).** `WINGGUY_CONNECTOR_MULTITENANT=1` + `WINGGUY_ENABLED_CLIENTS=Julian-Davis` set on prod. Verified: Julian's Portal Token resolves to `Julian-Davis` (Active), Guy still resolves to Guy (not locked out). NB the `ENABLED_CLIENTS` allow-list gates the **extension**, the multitenant flag gates the **connector** — both now on. At scale, move the allow-list off the env var onto a per-client record field (~3-4 clients).
- **★ BYO ANTHROPIC KEY: BUILT (Option A — key lives in the client's browser).** Decision 2026-07-13: a client's own Claude key is their BILLING credential, so it stays with them, never in Airtable. `config/anthropicClient.js` `getAnthropicClientForKey(apiKey)` (per-key client, falls back to platform key); the 3 wingguy drafting sites read `x-anthropic-key` via `byoAnthropicClient(req)`. Extension: a "Your Claude API key" field in the popup (chrome.storage.local only), sent per-request. Dormant-safe (no header → Guy's key). Julian gets the extension from day one, so this was on his critical path.
- **★ BILLING HARDENING (2026-07-14 morning).** The BYO fallback must NEVER silently bill the PLATFORM (Guy's) key for a client. `byoAnthropicClient(req)` in `routes/wingguyRoutes.js` now: client's own key (`x-anthropic-key`) → theirs; else platform key ONLY for the owner, a client with **`Managed Claude Key`=Yes** on their Client Master row (new single-select field → `clientService` maps `req.client.managedClaudeKey`), or the `WINGGUY_PLATFORM_KEY_CLIENTS` env override; else the 3 drafting sites return 400 "add your Claude key". Verified live: Guy→platform (owner), Julian (blank)→blocked. So a client is only on Guy's key when explicitly put on a managed plan (flip the field to Yes). Also corrected the install doc: free Claude DOES allow ONE custom connector (was wrongly stated "paid only").
- **★ JULIAN GO-LIVE STATE + SEQUENCE.** His foundation is READY (record Active, Portal Token, own leads base, timezone Brisbane). His email/calendar are both Zoho (`xapi.com.au` MX = Zoho Mail AU) → **email = Nylas over IMAP, calendar = the Zoho adapter**. Remaining steps: **(1) Connect his Claude** — send `https://pb-webhook-server.onrender.com/mcp2/<his Portal Token>` + `docs/wingguy-connector-install.md` message (needs Claude Pro/Max/Team/Ent). **(2) Rules** — he says "let's set up my rules" (wingguy_setup_rules seeds+personalises). **(1+2 = first wow, chat-only.)** **(3) Email** — connect his Zoho Mail to Nylas via IMAP (app-specific password) → Nylas Grant ID. **(4) Calendar** — send his Zoho link `/auth/zoho/start?clientId=Julian-Davis&token=<token>`. **(5) later** Fathom (his account + API key → the Fathom API Key field). Extension is BYO-ready now. Paused mid-Step-2 (about to pre-flight his connector).
- **★ CALENDAR PROVIDER MODEL (clarified for Guy this session, worth keeping):** Guy is a **grandfathered hybrid** — calendar READS via Google service-account (his `Google Calendar Email` is set), WRITES + email via Nylas. **Every NEW client is all-Nylas** (or Nylas-email + direct-cal-adapter like Zoho): rule = leave `Google Calendar Email` BLANK + set `Calendar Provider`. The `Nylas API Key` (shared) ∶ per-client `Nylas Grant ID` mirrors `ZOHO_CLIENT_ID/SECRET` (shared) ∶ per-client `Calendar Provider Token`. Nylas serves Google/Outlook/IMAP; Zoho calendar needs the direct adapter (Nylas can't).

**▶▶ 2026-07-13 SESSION 2 CLOSE — THE RULES-SETUP BUILD: part (a) DONE + part (b) steps 1-2 DONE & VERIFIED ON PROD.**
- **Part (a) - template-layer sweep: ✅ DONE.** The banked sweep note was STALE - verified against the LIVE store (prod dump via a Render job): the 10-Jul sweep had already caught template up on booking-defaults (v2, de-personalised `{{owner_last_name}}`) and timezone-playbook (v1, region-neutralised). The ONLY real gap left was **email-html-format** (template still taught the old Ctrl+K workaround) - swept to **template v2** (clean-link default; commit on prod store via the door). Confirmed: the 22 client-only rules are correctly Guy-only (scaffold-paired or his content), the 6 scaffolds intact. **No transferable-craft divergence remains.**
- **Part (b) step 1 - seed-then-diverge: ✅ BUILT + VERIFIED (commits 2b7a416b + 63c8f407).** `store.seedClientFromTemplate({tenantId})` + `scripts/wingguy-seed-client.js` (dry-run default, `--commit`). Copies every active template rule into a new tenant's client layer as v1, **idempotent** (rules the client already diverged on are skipped, never overwritten); variables (global catalog) + assets (client fills own) NOT seeded. Verified on a throwaway tenant: dry-run 58→ commit 58 → re-run skips 58 → runtime (foundation ∪ client) = **61** (vs the bare 3 before); cleaned up after (store pristine). Needed a DB-constraint migration so `wingguy_rule_history` accepts the `seed` action (existing table's CHECK predated it). Dormant until called.
- **Part (b) step 2 - `wingguy_setup_rules` tool: ✅ BUILT + VERIFIED (commit 2dadb0de).** Third onboarding sibling in `services/wingguyGetStartedMcp.js` (both transports auto-register from TOOL_DEFS). **State machine driven entirely by the store** - each call reads the caller's client layer, **lazily seeds from template if empty**, works out which `*-scaffold` rules are still unfilled, and hands the ambient Claude the NEXT beat (basics → angle → manifesto → targeting → objections → assets → call two). Division of labour: the TOOL does structural writes only (seed + **auto-retire a scaffold once its filled rule exists**); the ambient Claude does the generate/interview + content commit through the existing propose→commit door. **Anti-clone by omission: ships the pedagogy, never Guy's own angles/manifesto** (per the ANTI-CLONE principle: "ship prompts and craft, never Guy's content/examples"). Resumable + tenant-scoped. Verified end-to-end on prod via a job (monkeypatched getClientById): seed→basics→angle→commit→scaffold auto-retired→…→all 6 scaffolds reaped→"your rules are set up"; throwaway tenant wiped after. **Guy path proven safe: read-only, 75→75 (zero writes), returns "all set up"** (his layer has the FILLED keys, no scaffolds).
- **★ ANTI-CLONE DESIGN NAILED THIS SESSION (Guy's fractional worry, resolved):** worked examples (Guy's fractional/advocacy) are **shown as a live demo, then thrown away** - never committed to the client's rulebook. The beat says the quiet part out loud ("this angle's mine and it's crowded - take the shape, not the words"). What gets committed is THEIRS: angle *generated* from their business, manifesto *interviewed* out of their conviction. Julian can shape it however he wants. Key nuance: for non-coach clients (brokers/planners) Guy's example can't even be copied by context, so it only teaches shape; the saturation risk is real only for a fellow-coach client in Guy's own pond (there: different demo, or a hard differentiation check at go-live).
- **★ NEXT = step 3 + Julian go-live.** (1) **Wire seeding into provisioning** (`scripts/onboard-new-client.js` currently does the Airtable base + client record but NOT the rules store) - though **the tool's lazy-seed already makes Julian workable without this**: he can connect and just say "let's set up my rules" and it bootstraps. Explicit provision-time seed is a nicety (so get_started/vision see a seeded state pre-setup). (2) **Dry-run Julian end-to-end** on a real/synthetic Julian-state tenant. (3) **Julian go-live = Guy's call, minutes:** flip prod `WINGGUY_CONNECTOR_MULTITENANT=1`, send his `/mcp2/<his-Portal-Token>` URL. Zoho calendar adapter is separate optional-later (see [[project_julian_pilot_zoho_calendar]]).
- **⚠ Two notes:** (a) an MCP connector's tool list is cached at connection init - **Guy must reconnect his Wingguy connector to see `wingguy_setup_rules`** (it's registered server-side + deployed). (b) a pre-existing `smoke-test` throwaway tenant sits in the store (from `wingguy-rules-smoke.js`, not this session) - harmless, no runtime read uses it.

**▶▶ 2026-07-13 SESSION CLOSE — multi-tenant + onboarding SHIPPED. NEXT = the rules-setup build (fresh chat). START HERE.**
- **Shipped + verified this session (all on main + staging; prod safe, flag OFF):**
  - **Multi-tenant Stages 0-2:** per-client connector tenancy via the existing **Portal Token** (WorkOS dropped) — `resolveCoachClientId(:token)` behind `WINGGUY_CONNECTOR_MULTITENANT` (off on prod). Isolation test PASSED on staging (client token → own tenant, not Guy; random → 401; revocation works). The 3 shared leaks fixed (bookingPrefs personal fields, in-chat rules-door, owner-gate → allow-list). Detail in the 2026-07-12 block below.
  - **Onboarding tools (in-connector, state-aware):** `wingguy_vision` (the PRIMARY "what can I do with Wingguy?" answer — day-to-day flow, copy/paste-now vs optional Chrome-extension upgrade [BYO key or flat fee], setup folded in as "the good part", hours-scale-with-outreach) + `wingguy_get_started` (short status). Both verified live on staging. `services/wingguyGetStartedMcp.js`.
  - **Install handover** (the one out-of-connector step): `docs/wingguy-connector-install.md` — Guy pre-flight + copy-paste client message + troubleshooting.
- **Julian status:** record + base + Portal Token all READY. To put him live (Guy's call, minutes): (1) flip prod `WINGGUY_CONNECTOR_MULTITENANT=1`; (2) send him his `/mcp2/<token>` URL (per the install doc); (3) his calendar=Zoho (needs adapter), email + Fathom unknown — capture on his record.
- **★ NEXT = ZOHO CALENDAR ADAPTER FIRST (decided 2026-07-13), THEN the rules-setup session.** Guy chose Zoho-first: it's **independent dev work** (doesn't need Julian in the room, unlike the rules session), it's the **last plumbing gap**, and booking is guarantee-heavy so build+test it off to the side. Fresh chat: *"start the Zoho calendar adapter build."*
  - **GATING FIRST STEP = Guy registers a Zoho OAuth app** (Server-based Application, Zoho API console) → `ZOHO_CLIENT_ID` / `ZOHO_CLIENT_SECRET` (env; one app serves all Zoho tenants). Also need a **real Zoho calendar to test against** (Julian's or a scratch account). Nothing can be built/tested before the app exists (same shape as the WorkOS gating step was).
  - **Full checklist: `docs/zoho-calendar-adapter-plan.md`.** Real effort = (a) Zoho **data-centre/region routing** (per-account API host, don't hard-code) + (b) **`mapZohoEvent` edge cases** (recurring / timezone / all-day / RSVP / confirm guest-invite emails send). Everything above `calendarProvider.js` is untouched. **Adds 2 new client fields — `Zoho Refresh Token` + `Zoho API Domain` (+ `Calendar Provider`='zoho') — the ONLY new fields in this whole stream** (multi-tenant work added zero). Fields go on the Client Template base too.
- **★ THEN the RULES-SETUP session (needs Julian).** For the PILOT this is a **HANDS-ON working session** (us + Julian's business → author his rules via the "update my rules" door, **generate his worked examples from HIS business** = the anti-clone step), **NOT a big code build**. A polished **self-serve walkthrough** (client sets up their own rules unaided) is a **later, reusable** build — not needed for Julian. Prep = the **TEMPLATE-LAYER SWEEP**: Guy's best rules live in his CLIENT layer so a new client inherits almost nothing — de-personalise them into the shared TEMPLATE layer (values as {{variables}}). See ANTI-CLONE TEMPLATE PRINCIPLE + the door design below.
- **Watch-item (from 2026-07-13 provider Q&A):** calendar + email are INDEPENDENT and there's **no cross-check**. Calendar = the `Calendar Provider` field (google/nylas/[zoho]); email = the Nylas grant (email NEVER branches on Gmail-vs-Outlook — Nylas absorbs that). Today `calendarProvider.activeProvider` routes ONLY `'nylas'`→Nylas, **everything else (incl. an unbuilt `'zoho'`, or a typo) falls through to the GOOGLE path** → needs `googleCalendarEmail` → if absent, calendar silently fails while email keeps working. So a `Calendar Provider` that disagrees with what's actually connected = silent broken booking. **Worth a small provisioning/status guard** ("provider set but no matching credentials").

**▶▶ 2026-07-12 — ★ WorkOS DROPPED. Multi-tenant connector auth = the existing PORTAL TOKEN.**
- **The pivot (Guy's call, verified in code):** we do NOT need WorkOS / OAuth / a new login service. **Every client already has a per-client secret — the `Portal Token`** in the Master Clients Base ([clientService.js:133](../services/clientService.js)), a 24-char URL-safe random string Guy mints + rotates via `generate-portal-tokens.js` (`--force` regenerates = instant revoke). It ALREADY authenticates clients: the live portal takes `?token=`/`x-portal-token` → `getClientByPortalToken()` → attaches the full client (clientId, their base, their Nylas grant, their booking identity) — see [authMiddleware.js:245](../middleware/authMiddleware.js). So the "turn a per-client secret into *this is Julian*" job WorkOS was for **already exists and is in production.**
- **Both surfaces run on the ONE Portal Token.** The **Chrome extension already uses it** (sends `x-client-id` + `x-portal-token` on every backend call — [background.js:526](../wingguy-extension/background.js)); the extension→backend path is already per-client. The **MCP connector is the only piece without tenancy** — today `/mcp2/:token` accepts a single SHARED secret (`PB_WEBHOOK_SECRET`/`MCP_CONNECTOR_TOKEN`) and every tool executor hardcodes `RECALL_COACH_CLIENT_ID || 'Guy-Wilson'`. Stage 1 = make the connector resolve the Portal Token → clientId, thread it through.
- **Why safe for Guy:** the shared secret stays valid and maps to Guy-Wilson as the fallback, so **Guy's current connector URL is byte-identical and unchanged**. He doesn't even need a Portal Token for it. New clients get `/mcp2/<their-Portal-Token>` URLs. Additive; nothing of Guy's changes.
- **Cost/scope:** hours-to-a-day, not the ~1-1.5 weeks the WorkOS path was. No new account, no `.well-known` endpoints, no OAuth. WorkOS/AuthKit = shelved (only revisit if Wingguy ever needs *stranger self-serve signup*, which the managed-service model doesn't).
- **REVISED STAGED PLAN:**
  - **Stage 0 — ✅ DONE.** config/wingguyTemplates.js fallback confirmed in place (do NOT boat-burn during the build). WorkOS setup = deleted from the plan.
  - **Stage 1 — ✅ DONE + VERIFIED ON STAGING (2026-07-12, commit fc9a8ed0).** `mcpRecallServer.js` `resolveCoachClientId(:token)` — shared secret → Guy; else active-client Portal Token → that clientId; else 401. Resolved clientId threaded into transcript executors + `registerWingguyRulesTools/Booking/Mail(server, tenant)`, replacing the module `TENANT` const. Off-by-default flag `WINGGUY_CONNECTOR_MULTITENANT` (OFF on prod → Guy unchanged; ON on staging for testing). Legacy `/mcp` path + extension in-chat door call with 2 args → default Guy → unchanged.
    - **Staging isolation test PASSED:** throwaway `zzz-test-tenant` client + Portal Token (added/deleted via staging one-off jobs using the server's own Airtable creds). Flag ON. Logs proved `initialize`/`tools/call auth=ok tenant=zzz-test-tenant` (NOT Guy); `wingguy_rules_list` returned that tenant's OWN (empty) rulebook; random token → 401 (fail closed); after deleting the row the token → 401 again (revocation works). Prod re-verified live + unchanged (flag off).
    - **Not yet swept:** the booking + mail tools' actual data isolation on a tenant WITH a base (the rules path is proven; booking/mail need Stage-2 leak fixes first anyway). Staging left with the flag ON for that.
  - **Stage 2 — ✅ DONE (2026-07-12). Fixed the 3 shared leaks (affect BOTH surfaces); Guy's behaviour byte-unchanged (all fixes gate on non-Guy tenants only):**
    - **booking-prefs identity leak** ([wingguyBookingPrefs.js](../config/wingguyBookingPrefs.js)): the invite builder already reads `coach.bookingZoom || prefs.yourZoom` — the leak was the FALLBACK (Guy's personal Zoom/LinkedIn/phone). Fix: `getBookingPrefs` now BLANKS `yourZoom`/`coachLinkedIn`/`coachPhone` for any non-Guy tenant → a 2nd tenant's invite carries THEIR record fields or nothing, never Guy's. Time/load prefs stay shared defaults (real per-tenant values = the Postgres-store slice, later). Verified: Guy keeps his values, `Julian-Davis` → blank. **Follow-on (commit 5325a2a5): clientService was reading field names that DON'T EXIST in the base ('Booking Zoom Link'/'Coach LinkedIn URL'/'Coach Phone Number') so per-tenant booking identity ALWAYS came back null — corrected to the real fields `Meeting Link` / `LinkedIn URL` / `Phone` (the tenant is the "coach" on their own invites; the client's own contact fields ARE the booking identity — no new fields needed). Guy's three values verified identical to the DEFAULT_PREFS, so his invites unchanged; other tenants' details now actually flow.**
    - **in-chat rules-door leak** ([wingguyChat.js:429](../services/wingguyChat.js)): `legacyToolCall(name, input)` defaulted to Guy → any extension-chat "update my rules" wrote Guy's rulebook. Fix: pass `coach.clientId` (the caller's tenant).
    - **owner-gate** ([wingguyRoutes.js](../routes/wingguyRoutes.js)): single-owner 403 → CLOSED-by-default ALLOW-LIST (`ENABLED_CLIENTS` = owner + `WINGGUY_ENABLED_CLIENTS` env, comma-sep). Still owner-only until a client is explicitly added; onboarding Julian = add his clientId to the env var. Non-blocking cosmetic `coachName||'Guy Wilson'` fallbacks left as-is (provisioning fills the name).
  - **Stage 3 — ✅ CHECKPOINT: synthetic 2nd tenant on staging.** Mint a throwaway client + Portal Token; flip the flag on; deliberately try to leak into Guy's calendar/mailbox/rules.
  - **Stage 4-6 — Provision Julian → run him live beside Guy → Guy migrates last** (unchanged; TEMPLATE-LAYER SWEEP + Zoho calendar adapter still apply — see [[project_julian_pilot_zoho_calendar]]).
- **★ VA / roles token DATA MODEL (concretised 2026-07-12 — refines the "VA/roles mechanics" section below; still DESIGN, no code; builds when a VA is actually set up, AFTER Julian). Applies to Paul Faix + VA April (see [[project_paul_faix_fortix_april_va]]).**
  - **April gets her OWN row in the Master Clients Base** — NOT Paul's token (shared = indistinguishable → can't route her proposals to his approval queue). A thin **operator row**, reusing the Portal Token + the Stage-1 resolver we shipped today.
  - **Two new fields on the Clients table:** `Role` (`owner` / `va` / `platform`; blank/owner = today's behaviour) + **`Operates For`** (link to the client she runs). April's row = `Client ID: April-Fortix-VA · Portal Token: (her own) · Role: va · Operates For: Fortix · Status: Active`, with base/Nylas/calendar fields EMPTY (she borrows Paul's).
  - **Resolver grows one step (step-3 extension of `resolveCoachClientId`):** token → her row → `role=va`, `operatesFor=Fortix` ⇒ **whose data = Fortix** (Paul's calendar/mailbox/rules), **who's acting = April**, **role = va**. Everything works as normal EXCEPT a rule commit, which (role=va) parks in Paul's approvals queue (`wingguy_rule_proposals`, per the VA/roles mechanics section) instead of committing. Paul's row = `owner` → commits directly.
  - **Why same table, not a separate People table:** reuses the shipped Portal-Token lookup (no new plumbing), matches "mint = one row + one URL, revoke = delete the row" (near-zero admin). A separate People table is tidier on paper but more to build — not worth it. The `Role` field cleanly signals operator-vs-tenant.
  - **Not on Julian's path:** Julian is a solo owner (no VA) → one token, no Role/queue. This layer is only needed to set April up.
- **★ ONBOARDING = A STATE-AWARE EXPERIENCE INSIDE THE CONNECTOR (concept agreed 2026-07-12). The GTM insight + the next build.**
  - **The realisation:** the "here's what Wingguy unlocks for you" pitch can only live INSIDE the connector once it's hooked into the client's Claude — before that their Claude is blank and has nothing to describe. So onboarding/selling-the-vision physically belongs post-MCP-connect, in-chat.
  - **The message CHANGES STATE, and Wingguy should read the client's own provisioning to pick which:** *not-fully-set-up* → "here's what's already live + here's your remaining BLANKS (no mailbox yet, no calendar yet) + connect them to unlock X"; *fully-set-up* → "here's how to DRIVE me — try typing '…'". Same door, adapts. This IS the already-banked client-facing help design ("what can I do?" tour + "show me my rules → **lead with the blanks**") pointed at onboarding — extend that, don't reinvent.
  - **GTM positioning to keep (Guy, 2026-07-12):** the product is NOT "another AI chatbot" — it's **the connections underneath that pull it all together** (AI wired into the client's real calendar/mailbox/CRM/LinkedIn, running THEIR rules). That's the moat + the honest "real future of AI" pitch. Sell the DESTINATION to justify the bit of setup friction. Proof = Guy's real dogfooding numbers (hours saved, e.g. the Jul-6 datapoint: 37 personalised msgs to 20 people) + Julian's testimonial once he's live (prove on Julian FIRST, then the testimonial becomes the pitch — don't sell ahead of what reliably works). Audience staging: **prospects** (not connected) → a static vision PAGE (reuse the series / Mindset-Mastery look); **connected-partway / fully-set-up** → the in-chat state-aware guide.
  - **`wingguy_get_started` tool — ✅ BUILT + VERIFIED on staging (2026-07-12, commit 574c05be).** New module `services/wingguyGetStartedMcp.js`; introspects the caller's tenant (client record: nylasGrant / fathomApiKey / calendarProvider / bookingZoom; + store client-layer rule-count) → tailored "what's live / your blanks / how to drive me" guide + positioning + proof. Registered on /mcp2 (SDK) + legacy /mcp (list+call). Tested via a throwaway Julian-state tenant on staging: rendered the full guide with all four blanks (rules/mailbox/calendar/transcripts) surfaced, name inserted. It's the in-product embodiment of the onboarding concept + Julian's front door the moment he connects.
  - **`wingguy_vision` tool — ✅ BUILT + VERIFIED on staging (2026-07-13, commit a3c13ce6).** Companion "sell-the-destination" tool (triggers: "what will I be able to do?", "show me the full picture"): the full day-in-the-life of a fully-connected Wingguy (lead accepts → thanks note; replies → booking from real calendar; books → invite+reminders; after call → transcript+email; all on THEIR rules), the positioning + proof, THEN the setup folded in as a WARM benefit-led close (Guy's framing 2026-07-13, commit 8b9268b2): "just a little bit of setup - and the setup is where it gets good", state-aware asks (which calendar, which email client, note-taker → "the one we plug into is Fathom, and Fathom transcripts are currently free"), and RULES framed as the great part ("shape all your messages in plain English, keeps getting better, save hours a day"). Fully-connected tenant gets "you're already there". Shares `resolveState` with get_started. **Reworked 2026-07-13 (commit ed0e6249) → now the PRIMARY "what can I do with Wingguy" answer** (get_started narrowed to a short status check): approved copy adds **how-it-works-now = copy/paste from LinkedIn into Claude** (named plainly, not hidden) + the **OPTIONAL Chrome-extension upgrade** ("do a lot better", inside LinkedIn, no copy/paste; **BYO Anthropic key or a flat fee**; staying on just Claude+MCP is explicitly "great") + hours-saved-scales-with-outreach + a paste-a-lead CTA. Verified live on staging.
  - **NEXT (to actually put Julian live):** (1) flip prod `WINGGUY_CONNECTOR_MULTITENANT=1` (Guy's call — go-live switch; Guy's shared token still → Guy, proven safe); (2) hand Julian his `/mcp2/<his-Portal-Token>` URL (token already set on his record); (3) **seed Julian's rules** = the template-layer sweep (de-personalise Guy's client-layer rules → template → seed Julian's client layer + his business specifics) — the real content lift + anti-clone step. Optional-later per his wow-loop: Nylas mailbox, Zoho calendar adapter, Fathom key.

**▶▶ 2026-07-11 SESSION 2 CLOSE (multi-tenant STEP-3 scoped — SUPERSEDED 2026-07-12: WorkOS path dropped for Portal Token, see block above; the scoping of the connector's tenancy gap below is still accurate).**
- **Caching lever ✅ SHIPPED** (1h prompt-cache TTL, commit 307d4b16, live on prod). Detail in the COST note below. Cached prefix measured ~25.5k tokens; cache_read verified 100% on a back-to-back prod probe (byte-stable → no silent invalidator).
- **★ COST PICTURE CORRECTED (important — stop quoting $60-90).** The Anthropic dashboard shows **actual last-30-days = USD ~$20**, NOT $60-90. The $60-90 was a *projection* (recent daily pace × 30); the 30-day total is low because the first ~2 weeks were near-zero ramp. Recent PRE-change run-rate ~$1.5-2.5 USD/day, spiky (a $0.40 day next to a $2.50 day; one ~$4.70 spike = a heavy build/testing day). At the heaviest recent pace ~$40-60 USD/mo projected — and that's GUY (all-day user + dev/testing overhead a real client won't have). **Currency: dashboard is USD; ×1.5 → AUD.** Guy's price ceiling = **$30 AUD ≈ $20 USD**. A real client (lighter than Guy, post-cache) expected comfortably under that. **Watch next week's post-change bars vs Jul 7-9 (~$2-2.5/day) for the cache win.** See memory [[project_wingguy_cost_model]].
- **BYO-key finding:** BYO only touches the **EXTENSION** — the MCP-connector tools run on the client's OWN Claude, never call Guy's API key. So "BYO for Julian" = his extension drafting on his key; his connector chat is already his-own-Claude. Anthropic client is a single global (config/anthropicClient.js), 3 call sites (wingguyChat.js:210, wingguyRoutes.js:316/389), clientId available at all → a clean per-key-factory thread + fallback to Guy's key.
- **★ MULTI-TENANT (roadmap step 3) SCOPING — decision = BIG PATH, built BESIDE Guy (no interruption).** Guy chose the full shared multi-tenant build over the per-instance shortcut (per-instance = ~few days, dodges the connector build, doesn't scale — kept as the fallback option only). No-interruption is guaranteed by the LOCKED architecture ("Additive only — Guy's single-tenant setup untouched until he flips flags").
  - **The one big piece = the connector has NO tenancy today.** /mcp2 uses a shared token + a hardcoded `TENANT = process.env.RECALL_COACH_CLIENT_ID || 'Guy-Wilson'` baked into every tool executor (mcpRecallServer.js:32 · wingguyBookingMcp.js:24 · wingguyMailMcp.js:25 · wingguyRulesMcp.js:18). If Julian connected, EVERY tool call hits GUY's calendar/mailbox/rules/leads; two tenants can't coexist in one instance. This IS **roadmap step 3 (per-client connector tokens; WorkOS AuthKit = decided approach), paused for the sales push** — the multi-day core. Full slice ~**1-1.5 weeks**, dominated by this.
  - **Extension side ~80% ready** (routes already scope by req.client.clientId + airtableBaseId; rules store tenant-keyed). Blockers, all small-moderate: (a) owner-gate wingguyRoutes.js:42/56 (OWNER_CLIENT_ID='Guy-Wilson' → 403s everyone else); (b) **booking-prefs BUG** config/wingguyBookingPrefs.js:54-56 IGNORES its clientId arg and returns GUY's Zoom room / LinkedIn / phone — so ANY 2nd tenant's invites carry Guy's details until fixed (shape is per-tenant, value 100% Guy); (c) **in-chat rules-door leak** wingguyChat.js:427-430 → module TENANT=Guy, so extension-chat "update my rules" writes to GUY's rulebook (drafting-side rules READS are correctly scoped via tenantId:req.client.clientId — only the in-chat door leaks). Cosmetic-only fallbacks (coachName||'Guy Wilson' at wingguyChat.js:175/198, wingguyCalendar.js:294, guestBookingEventBuilder.js:77) = non-blocking. (Distilled from an Explore scoping pass this session.)
- **★ STEP-3 STAGED PLAN (execute in a fresh chat):**
  - **Stage 0 — Prep (zero risk):** KEEP config/wingguyTemplates.js fallback through the build (do NOT boat-burn first — it's the safety net); set up the WorkOS AuthKit app.
  - **Stage 1 — Connector per-client identity, behind a flag:** WorkOS token → clientId per request; thread clientId into the tool executors, replacing the module TENANT const; keep "default to Guy if unidentified" as the safety net so Guy's existing token → Guy, unchanged. Additive.
  - **Stage 2 — Fix shared bits (Guy's values as the DEFAULT so nothing of his changes):** booking-prefs per-tenant; in-chat rules-door uses the caller's clientId; owner-gate → allow-list.
  - **Stage 3 — ✅ CHECKPOINT: synthetic 2nd tenant on staging.** Throwaway fake client; deliberately try to leak into Guy's data. Catch bugs here, off to the side.
  - **Stage 4 — Provision Julian (parallel, real):** client record + his Airtable base + Nylas email grant + rules layer (do the banked TEMPLATE-LAYER SWEEP → generate his worked examples from HIS business; ship prompts not Guy's content) + voice/booking values + **Zoho calendar adapter** (Nylas can't serve Zoho calendar — see [[project_julian_pilot_zoho_calendar]] / docs/zoho-calendar-adapter-plan.md).
  - **Stage 5 — Julian LIVE on the new version; Guy stays on his current setup, untouched, side by side.**
  - **Stage 6 — ✅ CHECKPOINT: Guy migrates LAST, on his own timing,** with a one-switch fallback if anything looks off.
- **Boat-burning** (delete config/wingguyTemplates.js) stays deferred until AFTER the multi-tenant build settles.
- **NEXT = start a FRESH chat** ("where are we on Wingguy — start Stage 0 of the multi-tenant build") → Stage 0 (WorkOS AuthKit setup + confirm the config fallback is in place). This chat (caching + cost + BYO + step-3 scoping) is done.

**▶▶ 2026-07-11 (COST + CACHING analysis — caching DONE ✅; see SESSION 2 CLOSE above for the outcome). Historical detail:**
...
- **DO FIRST — CACHING-BUG CHECK: glance at today's (2026-07-11) cost — the first FULL day on the store** (the flip went live partway through 07-10, so the 10th is a mixed day). ~$2 ≈ caching landing, all fine, no trimming needed. A $4-5 spike = a silent cache invalidator (prefix changing per call) = a real bug worth fixing. Verify via `usage.cache_read_input_tokens` on a live `/chat` call (should be > 0).
  - **✅ DONE 2026-07-11 — caching-bug check PASSED, then 1h-TTL SHIPPED.** Method: a one-off Render prod job (see memory `reference_render_jobs_exec`) that re-renders `rulesSource.agentSystem('tks',{})` on two back-to-back raw Anthropic calls and prints `usage`. Result: SOURCE=store, prefix **~25.5k tokens** (75973 chars, byte-stable across re-renders → NO silent invalidator), `cache_read=25553` = 100% prefix hit on call 2. So the $4-5-spike scenario is conclusively ruled out. NOTE: couldn't pull the dashboard $ programmatically — needs an Anthropic ADMIN key (`sk-ant-admin`), only `RENDER_API_KEY` is available; the cache_read probe IS the substantive bug-detector, so the $ figure is confirmatory-only (ask Guy to eyeball if wanted). Then flipped `cache_control`→`{type:'ephemeral', ttl:'1h'}` on all 6 draft/reply/chat blocks (store + config) in `services/wingguyRulesSource.js` — **commit 307d4b16, deployed live on prod, re-probed clean** (renders fine with the new field, cache still lands). 1h TTL is GA on the first-party API — no beta header (confirmed via the claude-api skill). Optional-not-done: a >5min-idle soak to *directly* observe 1h survival (contract-guaranteed, not observed).
...
- **NEXT: (1) ✅ caching-bug check → (2) ✅ 1h-TTL flip shipped + confirmed → (3) ▶ NOW back to the Julian build (per-person tokens → Zoho calendar adapter → provisioning).**
