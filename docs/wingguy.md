...
6. **Day-of-week labelling bug (from the 2026-06-30 real runs).** Wingguy once wrote "Wednesday 2 July" for a date that was a Thursday (+ a stale event titled "Rebooked for Friday", never a Friday). A BUG to verify/fix, not a decision. The current slot formatter (`fmtSlot`, timezone-aware) looked correct in the last cloud test, so it may already be resolved in the times path; still needs a check of the booking/event-title path. (Surfaced while resolving #2; logged so it isn't lost.)
