...
- **REVISED STAGED PLAN:**
  ...
  - **Stage 1 — ▶ IN PROGRESS: connector per-client identity.** In `mcpRecallServer.js`, resolve `:token` → clientId (shared secret → Guy; else active-client Portal Token → that clientId; else 401). Thread the resolved clientId into the transcript executors + `registerWingguyRulesTools/Booking/Mail(server, tenant)`, replacing the module `TENANT` const. Behind an off-by-default flag `WINGGUY_CONNECTOR_MULTITENANT` (portal-token branch inert until flipped on staging). Legacy `/mcp` path unchanged (defaults to Guy).
