...
- **★ ONBOARDING = A STATE-AWARE EXPERIENCE INSIDE THE CONNECTOR (concept agreed 2026-07-12). The GTM insight + the next build.**
  ...
  - **`wingguy_vision` tool — ✅ BUILT + VERIFIED on staging (2026-07-13, commit a3c13ce6).** Companion "sell-the-destination" tool (triggers: "what will I be able to do?", "show me the full picture"): the full day-in-the-life of a fully-connected Wingguy (lead accepts → thanks note; replies → booking from real calendar; books → invite+reminders; after call → transcript+email; all on THEIR rules), the positioning + proof, THEN a state-aware numbered "here's what we need to do to get you there" (a fully-connected tenant gets "you're already there"). Shares `resolveState` with get_started. Verified on a Julian-state tenant: rendered the vision + his 4 real setup steps.
