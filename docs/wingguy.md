...
- **★ VERIFY ON PROD (read-only, after deploy):** `scripts/wingguy-multi-calendar-check.js <clientId>` via a Render job — fields → listCalendars → availability summary → listing incl. all-day. Run for **Guy-Wilson** (google unchanged + real all-day rendering) and **Julian-Davis** (real Zoho listCalendars via his current wrong-account grant — still a live token until he re-auths). Also `scripts/wingguy-list-calendars.js <clientId>` = the setup step that fills the fields.
- **NEXT:** (1) run the field script; (2) deploy + the two prod checks; (3) when Julian re-auths on `xapi.com.au`: diagnostic first (unchanged advice), then set his `Calendar Read IDs=all` + nominate his write calendar — the multi-calendar ghost the 2026-07-17 close warned about is now a FIELD EDIT, not a build; (4) same offer to Guy if he wants his personal calendar read (list emails explicitly — google can't `all`).
