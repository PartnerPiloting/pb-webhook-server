...
- **REVISED STAGED PLAN:**
  ...
  - **Stage 2 — ✅ DONE (2026-07-12). Fixed the 3 shared leaks (affect BOTH surfaces); Guy's behaviour byte-unchanged (all fixes gate on non-Guy tenants only):**
    - **booking-prefs identity leak** ([wingguyBookingPrefs.js](../config/wingguyBookingPrefs.js)): the invite builder already reads `coach.bookingZoom || prefs.yourZoom` — the leak was the FALLBACK (Guy's personal Zoom/LinkedIn/phone). Fix: `getBookingPrefs` now BLANKS `yourZoom`/`coachLinkedIn`/`coachPhone` for any non-Guy tenant → a 2nd tenant's invite carries THEIR record fields or nothing, never Guy's. Time/load prefs stay shared defaults (real per-tenant values = the Postgres-store slice, later). Verified: Guy keeps his values, `Julian-Davis` → blank.
