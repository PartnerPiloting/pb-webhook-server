...
- **★ STILL OPEN (need Guy, not just code):**
  ...
  - **Subbu Iyer's `signup_link` ledger row is likely spurious (folded 2026-07-18; needs Guy's go).** The pre-fix URL-twin bug logged `signup_link` when `cost_benefit_page` was drafted to him 17 Jul (the two keys share one URL). The code is fixed but the DATA row remains - left in place, a real "here's where to join" email to subiyer@gmail.com will be refused without `resend_ok`. Check `wingguy_asset_ledger` for (Guy-Wilson, subiyer@gmail.com, signup_link) and delete if it dates to that draft - deleting real data = explicit go.
