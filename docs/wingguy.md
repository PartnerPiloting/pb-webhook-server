...
- **★ NEXT SESSION = THE RULES-SETUP BUILD (the substantive piece; everything above PROMISES it).** Fresh chat: *"where are we on Wingguy — start the rules-setup build."* Two parts: **(a) the TEMPLATE-LAYER SWEEP** — Guy's best rules live in his CLIENT layer, so a new client inherits almost nothing; de-personalise them into the shared TEMPLATE layer (values as {{variables}}, per the banked BANKED PRE-ONBOARDING TASK note below). **(b) the guided "let's set up my rules" walkthrough** — explain the model simply (you never edit files; tell me in chat, I propose, you confirm), walk the scaffolds (targeting / manifesto / angles / objections / assets), and **generate the client's worked examples from THEIR business** (the anti-clone rule). See the ANTI-CLONE TEMPLATE PRINCIPLE + the door design below.
