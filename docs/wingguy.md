...
- **✅ FIXED + SHIPPED (be992e6b, 553db87e):**
  ...
  - **Transcripts (the highest-priority find):** `fathomRecordingIngested` answered *"has any row been filed?"* = **ATTEMPTED, not SUCCEEDED** → an empty-bodied shell **sealed itself in permanently** (webhook + poller skipped it forever) = how Kate Phillips (1 Jul) was lost past retention. Now requires a non-empty body → the poller retries. Plus: zero-line ingest logged at info as `(0 lines, 1 leads)` → now a warn; poll failures discarded their reason (only `failed=1` at info, no id, no why - plausibly where Julian's 17 Jul restart vanished) → now logged per-recording; **coverage sweep on the poller's existing 15-min heartbeat** (no new cron) + `lastCoverage` on poll status + `scripts/transcript-coverage-check.js` (read-only, on demand); `recall_latest_transcript` now serves the newest meeting that HAS a body and refuses to pass off a hollow header ("missing, not empty").
