...
- **★ ONBOARDING = A STATE-AWARE EXPERIENCE INSIDE THE CONNECTOR (concept agreed 2026-07-12). The GTM insight + the next build.**
  ...
  - **NEXT BUILD = a `wingguy_get_started` connector tool:** introspects the caller's tenant (client record: nylasGrant / fathomApiKey / calendarProvider / bookingZoom; + store rule-count for the tenant) and returns a tailored "what's live / your blanks / how to drive me" guide. Self-contained, testable on the staging multi-tenant rig, gives Julian a front door the moment he connects — and is the in-product embodiment of the onboarding concept. Then: flip prod `WINGGUY_CONNECTOR_MULTITENANT` (Guy's call) + seed Julian's rules (the template-layer sweep — the content lift).
