...
- **★ GO-LIVE = all that's left, and it's GUY's part + one live test:**
  1. Register the one Zoho **Server-based** OAuth app (any Zoho account you control). Scopes: `ZohoCalendar.calendar.READ,ZohoCalendar.event.ALL`. Authorized redirect URI: `https://pb-webhook-server.onrender.com/auth/zoho/callback`.
  2. Set env on prod (Render): `ZOHO_CLIENT_ID`, `ZOHO_CLIENT_SECRET`, and `ZOHO_REDIRECT_URI=https://pb-webhook-server.onrender.com/auth/zoho/callback`.
