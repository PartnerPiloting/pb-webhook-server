...
5. Scope selector: **Last 1 / 2 / 3 months** + **Test sample** (default **50**, up to **100**; **stratified across DATA-DRIVEN bands** — read up to 1,000 existing `AI Score`s, split into thirds *by rank* [bottom/middle/top third of this client's own people], draw ~equal from each. NOT fixed thresholds: scores are relative to each client's attribute setup, so bands must come from their real distribution. Tuning movement shows most in the mid tier).
...
4. **Test sample:** default **50**, choosable **up to 100**, drawn as a **stratified sample** across low / mid / high bands (~16/band at the default). **Bands are DATA-DRIVEN, not fixed thresholds** — absolute scores are meaningless across clients (one client's "high" might be 55%, another's 90%, depending on their attribute setup). So: read existing `AI Score` for up to **1,000** already-scored leads (cheap Airtable read, no AI), split into thirds **by rank** (bottom/middle/top third *of that client's actual people* — rank not score-range, so no band comes out empty when scores cluster), then draw ~equal from each. Self-calibrating to each client's own distribution. Bands are defined off the CURRENT (pre-rescore) scores — so the before→after shows how each tier moved.
