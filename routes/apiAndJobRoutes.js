...
router.post("/api/regenerate-my-token", async (req, res) => {
  ...
  try {
    ...
    // Always use production portal URL (main) - clients receive links that work in production
...
/**
 * GET /api/wp-user-lookup?wpUserId=123
 *
 * Looks up a WordPress user by ID and returns their email, display name,
 * first/last name, and current PMPro membership level. Used by the Onboard
 * Client form to verify the entered WP User ID matches the client being
 * onboarded.
 *
 * Response statuses (always 200 unless wpUserId is malformed):
 * - found: WordPress returned the user
 * - not_found: WordPress responded but the user ID does not exist (404)
 * - wp_unavailable: could not reach WordPress (timeout/network/5xx/credentials)
 */
router.get("/api/wp-user-lookup", async (req, res) => {
  const axios = require('axios');
  const logger = createLogger({ runId: 'WP_LOOKUP', clientId: 'SYSTEM', operation: 'wp_user_lookup' });

  const wpUserId = parseInt(req.query.wpUserId, 10);
  if (!wpUserId || isNaN(wpUserId) || wpUserId <= 0) {
    return res.status(400).json({
      status: 'error',
      error: 'wpUserId is required and must be a positive integer'
    });
  }
  ...
  try {
    ...
    if (!wpBaseUrl || !wpUsername || !wpPassword) {
      ...
      return res.json({
        ...
        error: 'WordPress credentials are not configured on the server'
      });
    }

    const authHeader = 'Basic ' + Buffer.from(`${wpUsername}:${wpPassword}`).toString('base64');
    const userUrl = `${wpBaseUrl.replace(/\/$/, '')}/wp-json/wp/v2/users/${wpUserId}?context=edit`;

    logger.info(`Looking up WP user ${wpUserId}`);
    ...
    try {
      ...
      if (err.response?.status === 404) {
        logger.info(`WP user ${wpUserId} not found`);
        return res.json({ status: 'not_found', wpUserId });
      }
      const code = err.response?.status;
      logger.warn(`WP lookup failed for ${wpUserId}: ${err.message}`);
      return res.json({
        status: 'wp_unavailable',
        error: code ? `WordPress returned status ${code}` : err.message
      });
    }

    // Fetch PMPro membership (best-effort — display only, never blocks the response)
    let membership = null;
    try {
      const pmproService = require('../services/pmproMembershipService');
      const result = await pmproService.checkUserMembership(wpUserId, { clientEmail: userData.email });
      if (result && result.levelId) {
        membership = {
          id: result.levelId,
          name: result.levelName || 'Unknown',
          expiryDate: result.expiryDate || null,
          isValid: !!result.hasValidMembership
        };
      } else if (result && result.error) {
        logger.info(`PMPro lookup returned error for ${wpUserId}: ${result.error}`);
      }
    } catch (pmproErr) {
      logger.warn(`PMPro lookup threw for ${wpUserId}: ${pmproErr.message}`);
    }
    ...
    logger.error('WP user lookup error:', error.message, error.stack);
    return res.json({
      status: 'wp_unavailable',
      error: error.message
    });
