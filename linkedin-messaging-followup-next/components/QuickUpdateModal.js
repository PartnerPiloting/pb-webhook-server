...
export default function QuickUpdateModal({ 
  ...
  const performSearch = async (query) => {
    ...
    try {
      const result = await lookupLead(searchTerm);
  ...
  // Auto-search when pasting a URL or email (these are explicit actions)
  useEffect(() => {
    if (!searchQuery) return;
    
    // Only auto-search for URLs or emails (paste detection)
    const isUrl = searchQuery.includes('linkedin.com/') || searchQuery.startsWith('http');
          ...
          <div className="mt-4 relative">
            ...
            <div className="relative">
              <input
                ...
                placeholder="Type name and press Enter, or paste LinkedIn URL/email..."
                className="w-full px-4 py-2 pr-28 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
              <div className="absolute right-3 top-2.5 flex items-center gap-2">
                {/* Search button - only show when there's a query and no selected lead */}
