...
export default function ClientCodeEntry({ error = null }) {
  const isOutage = error && (error.includes('temporary outage') || error.includes('database service (Airtable)'));
      ...
      <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8">
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">
            {isOutage ? 'Temporary Outage' : 'Access Required'}
          </h2>
          <p className="text-gray-600">
            {isOutage ? error : 'Please use the secure link your coach sent you to access this portal.'}
          </p>
        </div>
        ...
        {isOutage ? (
          <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
            <p className="text-sm text-blue-800">
              Your link is valid. Please try again in a few minutes.
            </p>
          </div>
        ) : (
          <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
