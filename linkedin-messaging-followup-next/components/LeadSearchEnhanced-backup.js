...
const LeadSearchEnhanced = ({ 
  ...
  // Handle search terms change (from tokenized chips)
  const handleSearchTermsChange = (newTerms) => {
    // Convert array of terms back to comma-separated string
    const termsString = Array.isArray(newTerms) ? newTerms.join(', ') : newTerms;
  ...
  const handleExport = async (type) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:3001'}/api/linkedin/leads/search?${params}`);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
          {/* Name Search - narrower, 1.5 columns */}
        ...
        {/* Search Summary - Only show active filters, no pagination info */}
