  ...
  return (
    <div className="w-full flex flex-col lg:flex-row gap-6">
      ...
      <div className="lg:w-1/4 w-full">
        ...
        {/* Quick Follow-up Date Update */}
        {selectedLead && (
          <div className="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
            <div className="text-sm font-medium text-gray-700 mb-2">
              Quick Update: {safeRender(selectedLead['First Name'])} {safeRender(selectedLead['Last Name'])}
            </div>
            <div className="flex gap-2">
              <input
                type="date"
                value={selectedLead['Follow-Up Date'] || ''}
                onChange={async (e) => {
                  const newDate = e.target.value;
                  try {
                    await handleLeadUpdate(selectedLead.id || selectedLead['Profile Key'], { 'Follow-Up Date': newDate });
                  } catch (error) {
              ...
              <button
                onClick={async () => {
                  try {
                    await handleLeadUpdate(selectedLead.id || selectedLead['Profile Key'], { 'Follow-Up Date': '' });
                  } catch (error) {
                    console.error('Clear date failed:', error);
                  }
                }}
