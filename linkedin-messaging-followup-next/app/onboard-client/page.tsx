...
const TIMEZONES = [
  ...
  { value: TIMEZONE_OTHER, label: 'Other (enter manually)' }
];

const PRESET_TIMEZONE_VALUES = TIMEZONES.filter(t => t.value !== TIMEZONE_OTHER).map(t => t.value);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (mode === 'add') {
        ...
        if (data.success) {
          setResult(data);
          // Token is generated by backend and returned in response
          if (data.portalToken) {
              ...
              <div className="grid gap-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      name="timezone"
                      value={PRESET_TIMEZONE_VALUES.includes(formData.timezone) ? formData.timezone : TIMEZONE_OTHER}
                      onChange={(e) => {
                        const v = e.target.value;
                        setFormData(prev => ({
                          ...prev,
                          timezone: v === TIMEZONE_OTHER ? '' : v
                        }));
                        setResult(null);
                      }}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    ...
                    {!PRESET_TIMEZONE_VALUES.includes(formData.timezone) && (
                      <input
                        type="text"
                        name="timezone"
                        value={formData.timezone}
                        onChange={handleChange}
                        placeholder="e.g., America/Chicago or Europe/Paris"
