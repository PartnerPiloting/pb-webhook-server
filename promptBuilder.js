...
/* ----------------------------------------------------------------
   buildPrompt – header + attribute bullets + schema + footer
----------------------------------------------------------------- */
async function buildPrompt() {
  const { positives, negatives } = await loadAttributes();

  const positivesTxt = Object.entries(positives)
    .map(([id, d]) => `• ${id}: ${d.label || d.description || ""}`)
    .join("\n");

  const negativesTxt = Object.entries(negatives)
    .map(([id, d]) => `• ${id}: ${d.label || d.description || ""}`)
    .join("\n");
...
/* ----------------------------------------------------------------
   extractExperience – array if present, else build from organization_* keys
----------------------------------------------------------------- */
...
function slimLead(profile) {
  return {
    ...
    about     : (
      profile.about ||
      profile.summary ||
      profile.linkedinDescription ||   // Joy fix
      ""
    ).trim(),
