/* ===================================================================
   ...
   • Filters negative_scores so only attributes whose score < 0 appear
   • Early guard for empty / undefined responses
=================================================================== */
...
function extractJson(raw) {
  if (!raw || typeof raw !== "string")
    throw new Error("Empty GPT response");          // ← new guard

  /* 1 — whole response might already be JSON ---------------------- */
  const direct = safeJsonParse(raw);
  if (direct && typeof direct === "object") return direct;

  /* 2 — look for fenced ```json … ``` block ----------------------- */
...
function callGptScoring(raw) {
  ...
  const out = {
    ...
    unscored_attributes : Array.isArray(j.unscored_attributes)
                           ? j.unscored_attributes : [],
  ...
  for (const [id, val] of Object.entries(negativesIn)) {
    ...
    if (score < 0) {
      ...
      out.attribute_reasoning[id] = reason;
    }
  }
