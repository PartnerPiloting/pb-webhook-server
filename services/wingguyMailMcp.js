...
const TOOL_DEFS = [
  ...
  },
  {
    ...
    description:
      'THE INSTANT MEMORY behind every queue person AND every meeting attendee — call this for "remind me about X", "jog X", "any emails from X?", "how did the call with X go?", "what did we agree?", and for EACH attendee during "prep me for today\'s meetings". Returns the PRE-BUILT dossier (no waiting): WHY YOU\'RE MEETING (how the meeting came about and what they said they wanted — the answer to "what is this call even about?" on a FIRST call), a full written RECAP of each of the last two calls (what it was for, what was actually said, personal detail, how it was left — the answer to "what happened last time?" on a SECOND call; relay the most recent one IN FULL when prepping, never compressed to a line), PERSONAL detail worth opening with, where the relationship actually stands, commitments each side made, suggested next move, meeting summaries from the transcript store, the full dated timeline (emails incl. calendar accepts/declines, LinkedIn messages), and the EMAIL RECORD read from the mailbox itself at build time: every email sent to this person (all their known addresses) with EVERY LINK in the body written out - so "did the links actually go out?" is answered by the payload; NEVER hand that check back to the human and never call the correspondence tools to re-verify it - plus the most recent outbound IN FULL (often carries the natural opening line for the call), commitments each way with whether they appear honoured, deferrals in the sender\'s own words, promised introductions, and an index of their replies. Everything under the dossier\'s [DEPTH] marker is for follow-up questions ("more on Steve"), not the morning brief — close each attendee with that offer instead of relaying the archive. Rebuilt automatically whenever the person\'s thread changes. When the prepared store has nothing, it falls back AUTOMATICALLY to a live mini-dossier built from the CRM record (LinkedIn thread, status, meetings) — so a miss means the person truly is not in the CRM, not merely outside the queue. Pass email too when you have one (e.g. from a calendar invite) — matching is surer and reaches Alt Emails.',
    zodSchema: {
