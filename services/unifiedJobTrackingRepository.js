...
async function updateJobTrackingRecord(params) {
  ...
  try {
    ...
    // Add any other custom fields from normalized updates
    // Use case-insensitive comparison to prevent duplicate fields when updates object has different casing
    const excludedKeys = ['status', 'end time', 'endtime', 'error', 'progress', 'items processed', 'itemsprocessed', 'notes', 'system notes', 'clientid', 'success rate'];
...
async function updateAggregateMetrics(params) {
  ...
  try {
    ...
    // and 'Total Posts Harvested' fields were removed from Job Tracking table as they are now calculated on-the-fly.
    ...
    clientRecords.forEach(record => {
      ...
      // aggregates['Total Posts Harvested'] += Number(record.get(CLIENT_RUN_FIELDS.TOTAL_POSTS_HARVESTED) || 0); - Removed 2025-10-02
