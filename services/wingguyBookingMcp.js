...
const TOOL_DEFS = [
  {
    ...
    description: 'What is actually ON the coach\'s calendar for a day or a range ("what\'s on today?", "what does my week look like?", "am I free Thursday afternoon?", "what\'s my next meeting?"). ALSO the FIRST call for the daily "prep me for today\'s meetings" / "prepare me for my 2pm" - start here to get the meetings, then call wingguy_dossier per attendee for the history, past emails and what was agreed last time. That phrase means the DIARY, never the follow-up queue. Reads their real calendar live, whichever provider they use (Google, Nylas or Zoho) — so this is the RIGHT tool for the coach\'s own diary, and works for every client. This shows what is BOOKED; to find times to OFFER A LEAD use wingguy_check_availability instead (that one applies their booking rules). Read-only — it never changes anything. Defaults to today.',
    zodSchema: {
