...
async function collectWaitingPeople(coachClientId, coach) {
  ...
  for (const m of rows) {
    for (const p of m.pending) {
      ...
      const cur = byEmail.get(p.email) || { email: p.email, name: p.name || null, meetings: 0, latest: null, latestTitle: null, meetingIds: [], extraOnly: true, with: null, quiet: false };
  ...
  for (const person of byEmail.values()) {
    ...
    if (person.role !== 'extra') { person.with = null; person.quiet = false; }
  }
