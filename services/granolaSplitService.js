 ...
 *          themSeconds, themCount, firstThemAt, speakerNames, verdict, reason }
 */
function planGranolaChunks({ segments, events }) {
  ...
  const evs = (events || [])
    .map((ev) => ({ ev, start: ms(ev.start), end: ms(ev.end) }))
  ...
  const chunks = [];
  const finish = (index, event, chunkSegs, contiguousStart = false) => {
  ...
  for (let i = 0; i < evs.length; i++) {
    ...
    finish(i + 1, evs[i].ev, chunkSegs, i > 0 && cuts[i - 1].contiguous);
  }
  return { span, chunks };
}
