---
slug: the-process
title: The whole process on one page
dek: "Everything, start to finish, at a glance. Don't try to master it - just see the shape of the journey you're on."
series: network-building-rethought
order: 12
status: draft
audience: insider-flavoured - client email #1 + library start-here page (NOT in the prospect drip)
---

# The whole process on one page

Most networking advice hands you a pile of tactics and no map. Go to the event. Hand out the cards. Post something. Follow up. Somehow, out of all that motion, work is supposed to appear - and when it doesn't, you assume you just need to try harder. But there was never any shape to it. Just effort, and hope.

This is the shape. The whole thing, start to finish, on one page - seven small, repeatable steps that carry you from "who do I even talk to?" all the way to work arriving on a recommendation.

Don't try to master it today. A map doesn't teach you the terrain - it just shows you where everything is, so you always know where you are and what comes next. Each step below has its own short piece that goes deep when you're ready for it.

## 1. Choose who you want in your network

The old way is to turn up wherever there's a room and work with whoever happens to be there. This flips it: you decide, on purpose, who belongs in your network before you meet a soul - people positioned to trust and recommend you, chosen for where they sit and how they think. Here's the real game-changer, though: the best of them won't just recommend you - they'll go on to build their own networks the same way you are, so a handful of the right relationships quietly multiplies into many. This was never a solo sport, and step seven is where it pays off.
→ *Choose who you want in your network*

## 2. Find them, and reach out

Now you go looking. LinkedIn lets you search the whole professional world and pull out exactly those people, and a short, genuine note goes out with the request - about their work, not yours. Choose well and a good share say yes; a healthy connection rate is your first sign the room was well chosen.
→ *Who do I even reach out to?*

## 3. Say a proper thank-you

The moment someone connects, you've got a small window of goodwill. Most people waste it on a copy-paste "thanks for connecting." You spend twenty seconds on something real and specific to them instead - and that's what turns a connection into an actual conversation.
→ *The 20-second thank-you*

## 4. Suggest a call - the right way

When the conversation's warm, you offer a couple of times that suit and do the arranging yourself. No "here's my link, grab a slot" - that quietly tells them they're a cog in your system. A little care here says the opposite: you matter, and I did the work.
→ *Why I'll never send you a Calendly link*

## 5. Have the discovery call

This is the easiest call in the world once you know what it's for: learning about them, not selling yourself. You find out what they do, how they think, and who they might one day connect you with. Nothing to pitch means nothing to be nervous about.
→ *What your first discovery call is actually for*

## 6. Give yourself a reason to follow up

This is where most good conversations quietly die - "let's stay in touch," with nothing to actually stay in touch about. A real relationship needs a reason to come back: something to share, something to work on, a next thing worth talking about.
→ *A reason to follow up*

## 7. Nodes - where it all compounds

Do this with enough of the right people and something shifts. You're not networking alone any more - you've got a web of people who all think this way, recommending each other, growing each other's networks. That's the whole game: not you chasing work, but work arriving on a recommendation.
→ *Nodes*

---

That's the journey. Seven steps, each one small enough to do well without it taking over your week.

If you're impatient, jump straight to whichever step you're on - each piece stands on its own. If you're not, they'll arrive one at a time, in order, and this map will be here whenever you want to see where you are.

<!-- WIP notes: First draft 2026-07-06, not reviewed by Guy. THE MAP (#12). Descriptive title is deliberate here - the map is a signpost, so it should say what it is (the one exception to the titles-provoke-the-penny-drop rule; the paradigm pieces still follow it). Serves double duty: client email #1 (right after welcome, before the journey pieces) AND the library start-here page; every footer's standing line points here. 7 steps per arc, framed against old-model "turn up, grind, hope". Connection-rate rung weaved into step 2 as a sentence (NOT its own step) since the #33-vs-bridge decision is still parked - if #33 becomes its own piece, promote it to step 3 here and renumber. Kept tech light-touch (implied, not named) so the one page is safe for a prospect who wanders in from a footer link. Step arrows (→ title) are placeholder links until the library exists. Shallow is the feature. -->
